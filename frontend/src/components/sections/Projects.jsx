...
import { FaRobot,FaDocker, FaHtml5, FaCss3Alt, FaPython } from "react-icons/fa";
import {
  SiFastapi,
  SiTensorflow,
  SiScikitlearn,
          ...
          <div className={`opacity-0 transition-all duration-500 ease-in-out ${isTapped ? 'opacity-100 translate-y-0 ':'opacity-0 translate-y-4'} md:opacity-0 md:group-hover:opacity-100 md:translate-y-4 md:group-hover:translate-y-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0`}>
            ...
            <div className="mb-4 flex flex-wrap items-center gap-3">
              {Array.from(groupedIcons.values()).map((info) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`text-3xl cursor-none transition-transform group-hover/tooltip:scale-110 ${displayColor} hover:text-green-400 hover:filter-drop-shadow-[0_0_10px_#00ff00]`}
                    >
                      {info.icon}
                    </div>
            ...
            <div>
              <a
                ...
                className="inline-block cursor-none font-semibold text-green-300 transition-colors hover:text-green-200"
              >
                View Project →
              </a>
            </div>
